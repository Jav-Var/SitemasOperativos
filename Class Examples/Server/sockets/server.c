...
int main(void) {
    int fd, fd2; // fd: file descriptor del server
                 // fd2: 
    ...
    fd = socket(AF_INET, SOCK_STREAM, 0); // Para UDP usar SOCK_DGRAM
    if (fd == -1) {
        perror("socket");
        exit(EXIT_FAILURE);
    }
    ...
    // Solucion 1 (Opciones de socket / Reusar direccion)
    int opt = 1;
    if (setsockopt(fd, SOL_SOCKET, SO_REUSEADDR, &opt, sizeof(opt)) == -1) {
        perror("setsockopt");
        /* no fatal: solo informamos */
    }
    // Buscar solucion (2) detectar Control+C (signal)

    memset(&server, 0, sizeof(server));
    server.sin_family = AF_INET;
    server.sin_addr.s_addr = INADDR_ANY;
    server.sin_port = htons(PORT); // htons convierte al byte order apropiado
